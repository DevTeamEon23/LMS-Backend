...
def fetch_enroll_group_for_inst_learner(user_id: int, admin_user_id: int):
    try:
        # Fetch all enrolled users' data of group here
        groups = fetch_group_to_enroll_to_inst_learner(user_id, admin_user_id)
...
def fetch_userenroll_courses_for_inst_learner(course_id: int, admin_user_id: int):
    try:
        # Fetch all enrolled users' data of group here
        users = fetch_users_enroll_to_inst_learner(course_id, admin_user_id)
...
################################## Fecth enroll users of groups to Instructor and Learner ####################################################
    
@service.get("/fetch_users_of_group_enrolled_for_inst_learn")
def fetch_groupenroll_users_for_inst_learner(group_id: int, admin_user_id: int):
    try:
        # Fetch all enrolled users' data of group here
        users = fetch_enrollusers_of_group_to_inst_learner(group_id, admin_user_id)
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={
            "status": "failure",
            "message": "Failed to fetch enrolled users' data of group"
        })
